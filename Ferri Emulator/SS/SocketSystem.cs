...
namespace Ferri.Kernel.Network
{
    public class SocketSystem
    {
        ...
        public void Serialize(IPAddress IP, int Port, int Backlog, int SupportedAmount)
        {
        ...
        public void ProcessReceive(SocketAsyncEventArgs Args)
        {
            if (Args.BytesTransferred > 0 && Args.SocketError == SocketError.Success)
            {
                ...
                Buffer.BlockCopy(BufferPool.Buffer, Args.Offset, Data, 0, Args.BytesTransferred);

                if (Data[0] == 60)
                {
                    (Args.UserToken as Session).Socket.Send(Encoding.Default.GetBytes("<?xml version=\"1.0\"?> " +
                ...
                {
                    (Args.UserToken as Session).ReceiveData(Data);
                }
